...
module gopro_mount_female(length=1, base=false) {
    length = max(length, 1);
    translate([0, 0,-7.85]) linear_extrude(3.1) gopro_mount_profile();
    translate([0, 0,-1.55]) linear_extrude(3.1) gopro_mount_profile();
    translate([0, 0, 4.75]) linear_extrude(3.1) gopro_mount_profile();
